<section class="our_client">
	<hr class="soften"/>
	<h4 class="title cntr"><span class="text">Nhà Tài Trợ</span></h4>
	<hr class="soften"/>
	<div class="row">
		<?php
			$arr_brand=DB::select("select * from shop_brand");
			$count = 0;
			foreach($arr_brand as $rows_brand)
			{
				$count++;
		 ?>
		<div class="span2" {{ ($count>6)?'style=margin-top:10px;':'' }}>
			<a href="{{ $rows_brand->brand_link }}"><img alt=""  src="{{ asset('assets/img/brand/'.$rows_brand->brand_img) }}"></a>
		</div>
		<?php } ?>
...
<div class="container">
	...
	<span>Copyright &copy; 2017<br> Fudu's WebShop </span>
</div>
</div>
